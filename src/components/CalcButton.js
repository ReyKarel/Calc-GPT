...
import { addDigit, changeOperation, clearAll, deleteDigit, addDecimal, getResult } from "../store/slices/buttonsSlice";
...
const CalcButton = ({ digit, operation, children }) => {
    ...
    if (operation) return <button onClick={() => dispatch(changeOperation(operation))}>{children}</button>;
    switch (children) {
